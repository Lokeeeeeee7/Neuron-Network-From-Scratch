...
class Config(object):
    def __init__(self, num_epoch=1000, batch_size=200, learning_rate=0.0001, momentum_rate=0.9, epochs_to_draw=10, reg=0.00015, num_train=1000, visualize=True):
...
class Layer(object):
    ...
    def backward(self, x, delta_dot_w_prev):
        ...
        # [TODO 1.2]
        if(self.activation == 'sigmoid'):
            delta = sigmoid_grad(x @ self.w) * delta_dot_w_prev
            w_grad = x.T @ delta 
        
        elif(self.activation == 'tanh'):
            delta = tanh_grad(x @ self.w) * delta_dot_w_prev
            w_grad = x.T @ delta

        elif(self.activation == 'relu'):
            delta = reLU_grad(x @ self.w) * delta_dot_w_prev
            w_grad = x.T @ delta
...
class NeuralNet(object):
    ...
    def compute_loss(self, y, y_hat):
        ...
        data_loss = -np.sum(y*np.log(y_hat))/y.shape[0]

        # Estimating regularization loss from all layers
        reg_loss = 0.0
        for i in range(len(self.layers)):
            reg_loss += self.reg*np.sum(self.layers[i].w*self.layers[i].w)/2
        data_loss += reg_loss
    ...
    def backward(self, y, all_x):
        ...
        grad_last = self.layers[-2].output.T @ delta_L + self.reg * self.layers[-1].w
...
def minibatch_train(net, train_x, train_y, cfg):
    ...
    one_hot_y = create_one_hot(train_y, net.num_class)
    all_loss = []

    for e in range(cfg.num_epoch):
        for i in range(int(train_x.shape[0]/cfg.batch_size)):
            train_set_x = train_x[i*cfg.batch_size:(i+1)*cfg.batch_size].copy()
            train_set_y = one_hot_y[i*cfg.batch_size:(i+1)*cfg.batch_size].copy()
...
def bat_classification():
    ...
    cfg = Config(num_epoch=500, learning_rate=0.0001, num_train=train_x.shape[0], momentum_rate=0.9, batch_size=200)
...
def mnist_classification():
    ...
    # Define hyper-parameters and train-related parameters
    cfg = Config(num_epoch=300, learning_rate=0.0001, batch_size=200, num_train=train_x.shape, visualize=False)
